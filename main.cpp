#include <iostream>
#include "CalcNumBatterysForDayPowerConsumption.h"
#include "CalcNumSolarPanelsForDayPowerConsumption.h"

static void processBatterys( std::istream& is )
{
	CalcNumBatterysForDayPowerConsumption::wHDay totalPowerConsumption{ };
	std::cout << "Input total power consumption ( wHDay )" << '\n';
	is >> totalPowerConsumption;

	std::cout << '\n';

	CalcNumBatterysForDayPowerConsumption::AH batteryCapacity{ };
	...
	CalcNumBatterysForDayPowerConsumption::BatteryType batteryType{ };

	if( numBattteryType == 1 )
		batteryType = CalcNumBatterysForDayPowerConsumption::BatteryType::Lead_acid;
	else if( numBattteryType == 2 )
		batteryType = CalcNumBatterysForDayPowerConsumption::BatteryType::Li_ion;
	else
		throw std::invalid_argument( "unknown battery type" );

	CalcNumBatterysForDayPowerConsumption result( totalPowerConsumption, batteryType, batteryCapacity );
...
{
	CalcNumSolarPanelsForDayPowerConsumption::wHDay totalPowerConsumption{ };
	std::cout << "Input total power consumption ( wHDay )" << '\n';
	is >> totalPowerConsumption;
	...
	CalcNumSolarPanelsForDayPowerConsumption result( totalPowerConsumption, numOfSunshineHours );
